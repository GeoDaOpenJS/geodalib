...
test('Test localG()', async (t) => {
  ...
  t.deepEqual(
    result.lisaValues,
    [
      0.10526315789473684, 0.10526315789473684, 0, 0.36666666666666664,
      0.3723404255319149, 0.3695652173913043,
    ]
  );
...
test('Test localG() with GStar', async (t) => {
  ...
  t.deepEqual(
    result.lisaValues,
    [
      0.09523809523809523, 0.09523809523809523, 0, 0.2698412698412698,
      0.2698412698412698, 0.2698412698412698,
    ]
  );
  t.deepEqual(result.clusters, [2, 2, 4, 0, 0, 0]);
  t.end();
