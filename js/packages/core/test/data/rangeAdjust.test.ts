...
describe('Range Adjust Tests', () => {
  ...
  describe('rangeAdjust', () => {
    test('should adjust basic range to [0, 1]', async () => {
      const data = [1, 2, 3, 4, 5];
      const result = await rangeAdjust(data);
    ...
    test('should handle large dataset', async () => {
      ...
      // Should have same length and be properly range adjusted
    ...
    test('should handle very small numbers', async () => {
      ...
      // Should be properly range adjusted to [0, 1]
      const expected = [0, 0.5, 1];
      expectArraysNearEqual(expected, result);
    });
