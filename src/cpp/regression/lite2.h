...
enum WeightsType { W_UNDEF, W_MAT, W_GWT, W_GAL };
...
class ShortIterator {
 public:
  ...
  ShortIterator &operator-=(const size_t decrement) {
    ptr -= decrement;
    return *this;
  }
...
class Iterator : public ShortIterator<T> {
 public:
  ...
  explicit Iterator(T *init = NULL, T *last = NULL) : ShortIterator<T>(init), end(last) {}
...
struct pairstruct {
  ...
  bool operator<(const pairstruct<First, Second> &x) { return first < x.first; }
...
class Vector : public ShortIterator<T> {
 public:
  ...
  Vector(const size_t initsize = 0) {
    alloc(initsize);
    return;
  }
  Vector(const size_t initsize, const T initvalue) {
    alloc(initsize, initvalue);
    return;
  }
...
void Vector<T>::fill(const size_t initsize, const T initvalue) {
  ShortIterator<T>::ptr = begin;
  for (size_t cnt = initsize; cnt; --cnt) *(ShortIterator<T>::ptr)++ = initvalue;
  return;
}
...
void Vector<T>::Swap(Vector<T> &with) {
  ...
  int t;
  begin = with.begin;
  with.begin = buffer;  // swap begin
  buffer = ShortIterator<T>::ptr;
  ShortIterator<T>::ptr = with.ptr;
  with.ptr = buffer;  // swap ptr
  buffer = end;
  end = with.end;
  with.end = buffer;  // swap end
...
Pair *Find(Iterator<Pair> it, const Key What) {
  ...
  do {
    ...
    if (Median->first < (int)What)
      Sta = Median + 1;
    else if (Median->first > (int)What)
      Sto = Median - 1;
    else
      Sta = Sto = Median;
  } while (Sta < Sto);
...
Pair *FindW(Iterator<Pair> it, const Key What) {
  ...
  do {
    Median = Sta + (Sto - Sta) / 2;
    if (Median->first < (int)What.first)
      Sta = Median + 1;
    else if (Median->first > (int)What.first)
      Sto = Median - 1;
    else
      Sta = Sto = Median;
  } while (Sta < Sto);
 ...
 */
// template <class R>
inline void ColumnMultiply(Iterator<VALUE> column, Iterator<WMap> mt, Vector<VALUE> &product) {
  product.reset();
  for (; mt; ++mt) {
    VALUE TmpResult = 0;
    Iterator<WPair> it = (*mt)();
    for (; it; ++it) TmpResult += column[(*it).first] * (*it).second;
    product << TmpResult;
  };
  return;
}

inline int NeighborId(const Iterator<WPair> it) { return (it) ? (*it).first : INT_MAX; }
