...
extern "C" int dgeev_(char *jobvl, char *jobvr, integer *n, doublereal *a, integer *lda, doublereal *wr, doublereal *wi,
                      doublereal *vl, integer *ldvl, doublereal *vr, integer *ldvr, doublereal *work, integer *lwork,
                      integer *info);
extern "C" int dgesv_(integer *n, integer *nrhs, doublereal *a, integer *lda, integer *ipiv, doublereal *b,
...
void RowStandardize(Iterator<WMap> mt) {
  for (; mt; ++mt) {
    ...
    if (rowsum != 0)  // avoid standardizing empty rows
      for (Iterator<WPair> it = (*mt)(); it; ++it) (*it).second /= rowsum;
  };
}
...
void RowStandardize(Iterator<WVector> it) {
  ...
  for (int row = 0; row < dim; ++row) {
    ...
    if (rowSum != 0)  // avoid empty rows
      for (int col = 0; col < dim; ++col) it[row][col] /= rowSum;
  };
}
...
VALUE OnePoly(Iterator<V> Poly, const VALUE Val, const INDEX Prec, INDEX Pr) {
  ...
  for (; Pr < Prec; ++Pr) Power[Pr] = 0;

  for (it = Poly; it; ++it) {
    Iterator<VALUE> Po = Power();
    S = 0;

    for (itr = (*it)(); itr; ++itr, ++Po) S += *itr * *Po;
...
struct TraceProxy {
  ...
  TraceProxy(const VALUE a0 = 0, const VALUE a1 = 0) : S0(a0), S1(a1), Gamma0(0), Gamma1(0){};
};
...
TraceProxy::TraceProxy(const VALUE *Trace, const INDEX Prec) {
  ...
  if (Trace[0] * Trace[2] <= 0) {
    ...
    Gamma0 = sqrt(fabs(Trace[0] / Trace[2])) + 0.5 / Prec;
  if (Gamma0 > 1) Gamma0 = 1;
  S1 = -Trace[1];
  if (Trace[1] * Trace[3] <= 0) {
    S1 = 0;
    Gamma1 = 1;
  } else
    Gamma1 = sqrt(fabs(Trace[1] / Trace[3])) + 0.01 / Prec;
  if (Gamma1 > Gamma0) Gamma1 = Gamma0;
  return;
}

template <class IT>
void CrossConditioning(Iterator<IT> row, VALUE rowSqrtSum, Iterator<VALUE> sqrtSums) {
  for (; row; ++row) (*row).second /= (rowSqrtSum * sqrtSums[(*row).first]);
}
...
//           if (g[g[cnt].elt(cp).nbx].elt(ct).weight == g[cnt].elt(cp).weight) {
//             flag = true;
//             break;
//           }
//         }
...
double Estimate(Iterator<WVector> Poly, const double val, const INDEX Prec, const INDEX np) {
  ...
  if (fabs(ds21 - ds10) < 1e-14)
    ds = 0;  // if differences are too small, Aitken is meaningless
  else
    ds = ds21 * ds21 / (ds21 - ds10);  // Aitken's formula to compute the correction
...
double PrimeEstimate(Iterator<WVector> Poly, const double val, const INDEX Prec, const INDEX np) {
  ...
  const double ds21 = s2 - s1, ds10 = s1 - s0;
  if (fabs(ds21 - ds10) < 1e-14)
    ds = 0;  // bypass Aitken, if differnce is too small
  else
    ds = ds21 * ds21 / (ds21 - ds10);  // Aitken's formula
  return (s2 - ds);
}
...
void ScalProd(const CNT stop, Iterator<VALUE> v1, Iterator<VALUE> v2, const VALUE t = 1) {
  ...
  for (CNT elt = 0; elt < stop; ++elt) ValueAtStop -= v1[elt] * v2[elt];
...
void ScalProd1(const CNT start, const CNT stop, Iterator<VALUE> v1, Iterator<VALUE> v2) {
  ...
  for (CNT elt = start + 1; elt < stop; ++elt) ValueAtStart += v1[elt] * v2[elt];
 ...
 * one of the functions for computing LDL decomposition
 */
void ScalProd2(const CNT start, const CNT stop, Iterator<VALUE> v1, Iterator<VALUE> v2, Iterator<VALUE> diag,
               const CNT first) {
  VALUE ValueAtStart = diag[start];
  if (start != first) ValueAtStart *= v2[start];
  for (CNT elt = start + 1; elt < stop; ++elt) ValueAtStart += diag[elt] * v1[elt] * v2[elt];
...
bool SymMatInverse(Iterator<WVector> mt) {
  ...
  for (row = 0; row < dim; ++row) {
    for (column = 0; column < row; ++column) diag[column] = mt[column][column] * mt[row][column];
    ScalProd(row, mt[row](), diag());
    current = mt[row][row];
    if (fabs(current) < SL_SMALL) return 0;  // (near) singularity
    current = 1.0 / current;
    for (column = row + 1; column < dim; ++column) ScalProd(row, mt[column](), diag(), current);
  };

  // 2. compute inv(L')
  for (row = 1; row < dim; ++row)
    for (column = 0; column < row; ++column) ScalProd1(column, row, mt[row](), mt[column]());
  ...
  for (row = 0; row < dim; ++row)
    for (column = row; column < dim; ++column) ScalProd2(column, dim, mt[column](), mt[row](), diag(), row);
  // 4.b  do the upper triangular part of A
  for (row = 1; row < dim; ++row)
    for (column = 0; column < row; ++column) mt[column][row] = mt[row][column];
...
double normalize(WIterator arow, double &OD, const int row) {
  ...
  double h = 0, scale = 0;
  int cnt = 0;
  for (cnt = 0; cnt < row; ++cnt) scale += fabs(arow[cnt]);

  if (scale == 0.0 || last == 0) {  // skip the transformation
    OD = arow[last];                // off-diagonal is either 0 or arow[0]
    return 0;
  };
  for (cnt = 0; cnt < row; ++cnt) arow[cnt] /= scale;
  for (cnt = 0; cnt < row; ++cnt) h += geoda_sqr(arow[cnt]);
...
void householder(Iterator<WVector> a, double *diag, double *off) {
  ...
  for (row = dim - 1; row > 0; --row) {
    ...
    if (scale > 0) {
      ...
      for (j = 0; j < row; ++j) {
        double g = 0;
        for (cp = 0; cp <= j; ++cp) g += a[j][cp] * a[row][cp];
        for (cp = j + 1; cp < row; ++cp) g += a[cp][j] * a[row][cp];
        w[j] = g / scale;
        f += w[j] * a[row][j];
      };
      ...
      for (j = 0; j < row; ++j) {
        ...
        w[j] -= hh * f;
        double g = w[j];
        for (cp = 0; cp <= j; ++cp) a[j][cp] -= (f * w[cp] + g * a[row][cp]);
      };
    };
  };

  for (int cnt = 0; cnt < dim; ++cnt) diag[cnt] = a[cnt][cnt];
...
void SpatialLag(Iterator<WMap> mt, Iterator<WVector> var, WMatrix &lag) {
  ...
  for (int cnt = 0; cnt < (int)var.count(); ++cnt) {  // make sure each variable is correct
    if (lag[cnt].count() != var[cnt].count()) lag[cnt] = new WVector(var[cnt].count());
    lag[cnt].reset();
  };

  for (; mt; ++mt)  // compute spatial lag: treat each
    for (int cnt = 0; cnt < (int)var.count(); ++cnt) lag[cnt] << Product((*mt)(), var[cnt]());
}
...
void copy(WMatrix &dst, Iterator<WVector> src) {
  ...
  if (dim > 0) {
    dst.alloc(dim);
    for (int row = 0; row < dim; ++row) dst[row].copy(src[row]());
    dst.reset(dim);
  };
}
...
VALUE CL(WVector &resid, WVector &residW, const VALUE rho) {
  ...
  double sse1 =
      -1.0 * ((nn / 2.0) * (log(2.0 * M_PI)) + (nn / 2.0) * log((product / nn)) + (product / (2.0 * (product / nn))));
...
void _minus(WIterator a, WIterator b, WVector &r, const VALUE s) {
  if (a.count() != r.count()) r.alloc(a.count());
  r.reset();
  for (; a; ++a, ++b) r << (*a - *b * s);
}
...
void _minus(Iterator<WVector> a, Iterator<WVector> b, WMatrix &m, const VALUE s) {
  ...
  for (cnt = 0; cnt < (int)a.count(); ++cnt) {  // make sure each variable has correct size
    if (m[cnt].count() != a[cnt].count()) m[cnt] = new WVector(a[cnt].count());
    m[cnt].reset();
  };

  for (cnt = 0; cnt < (int)a.count(); ++cnt)
    for (int obs = 0; obs < (int)a[cnt].count(); ++obs) m[cnt] << (a[cnt][obs] - b[cnt][obs] * s);
}
...
VALUE ErrorLogLikelihood(Iterator<WVector> X, Iterator<WVector> lagX, WIterator y, WIterator lagY, Iterator<WMap> W,
                         const VALUE lambda, WVector &egls) {
...
VALUE GoldenSectionError(const VALUE left, const VALUE middle, const VALUE right, const WMatrix &X, const WVector &y,
                         Iterator<WMap> W, double *&beta, double *LogLik) {
  ...
  *LogLik = f1 - n / 2.0 - n / 2.0 * log(2.0 * M_PI);
  ...
  for (int cnt = 0; cnt < (int)egls.count(); ++cnt) beta[cnt] = egls[cnt];
  return x1;
}

VALUE GoldenSectionLag(const VALUE left, const VALUE middle, const VALUE right, WVector &resid, WVector &residW,
                       double *LogLik) {
  const VALUE GoldenRatio = (sqrt((double)5) - 1) / 2, GoldenToo = 1 - GoldenRatio;
  VALUE x0, x1, x2, x3, f0, f1, f2, f3;
  x0 = left;
...
void extract(const SparseVector &v, const double *scale, const int row, double &trace, double &trace2,
             double &frobenius) {
  double t2 = 0, fr = 0, t;
...
void extract(const DenseVector &v, const double *scale, const int row, double &trace, double &trace2,
             double &frobenius) {
  double t2 = 0, fr = 0;
...
void ScalProd(const int stop1, double *v1, double *v2, const double t = 1) {
  ...
  for (int elt = 0; elt < stop1; ++elt) ValueAtStop -= v1[elt] * v2[elt];
...
void ScalProd(const int stop1, double **v1, double *v2, const double t = 1) {
  ...
  for (int elt = 0; elt < stop1; ++elt) ValueAtStop -= v1[stop1][elt] * v2[elt];
...
void ScalProd1(const int start1, const int stop1, double *v1, double *v2) {
  ...
  for (int elt = start1 + 1; elt < stop1; ++elt) ValueAtStart += v1[elt] * v2[elt];
  v2[stop1] = -ValueAtStart;
  return;
}

void ScalProd2(const int start1, const int stop1, double *v1, double *v2, double *diag, const int first) {
  double ValueAtStart = diag[start1];
  if (start1 != first) ValueAtStart = ValueAtStart * v2[start1];

  for (int elt = start1 + 1; elt < stop1; ++elt) ValueAtStart += (diag[elt] * v1[elt] * v2[elt]);
...
void CopyMatrix(double **m, double **&a, int dim1, int dim2) {
  ...
  for (int i = 0; i < dim1; i++) {
    a[i] = new double[dim2];
    for (int j = 0; j < dim2; j++) a[i][j] = m[i][j];
  }
}

void CopyVector(double *&a, double *b, long n) {
  a = new double[n];
  for (int cnt = 0; cnt < n; ++cnt) a[cnt] = b[cnt];
}

double sum(int stop, double v1[], double diag[]) {
  double s = v1[stop];
  for (int cnt = 0; cnt < stop; ++cnt) s -= geoda_sqr(v1[cnt]) * diag[cnt];
  return s;
}

void makeSum2(int stop, double v1[], double v2[], double diag[]) {
  double s = v1[stop];
  for (int cnt = 0; cnt < stop; ++cnt) s -= v1[cnt] * v2[cnt] * diag[cnt];
  v1[stop] = s / diag[stop];
}

void product1(int start, int stop, double v1[], double v2[]) {
  double valueAtStart = v1[start];
  for (int cnt = start + 1; cnt < stop; ++cnt) valueAtStart += v1[cnt] * v2[cnt];
  v2[stop] = -valueAtStart;
}
...
void product2(int start, double v1[], double v2[], int first, double diag[], int dim) {
  ...
  if (start != first) valueAtStart *= v2[start];
  for (int cnt = start + 1; cnt < dim; ++cnt) valueAtStart += diag[cnt] * v1[cnt] * v2[cnt];
...
void residual(const DenseVector &rhs, const DenseVector *X, const DenseVector &ols, DenseVector &resid) {
  ...
  resid.alloc(dim);
  resid.copy(rhs);
  for (int cnt = 0; cnt < vars; ++cnt) resid.addTimes(X[cnt], -ols.getValue(cnt));
}
...
bool ordinaryLS(DenseVector &y, DenseVector *X, double **&cov, double *resid, DenseVector &ols) {
  ...
  for (row = 0; row < vars; row++) temp[row] = y.product(X[row]);
...
#else
  dgesvd_(&jobu, &jobvt, (integer *)&m, (integer *)&n, (doublereal *)a, (integer *)&lda, (doublereal *)s,
          (doublereal *)u, (integer *)&ldu, (doublereal *)vt, (integer *)&ldvt, (doublereal *)work, (integer *)&lwork,
          (integer *)&info);
#endif
...
double LcPrime(const DenseVector &v, const double *residW, const SparseMatrix &w, const double rho) {
  ...
  for (int cnt = 0; cnt < dim; ++cnt) rhs.plusAt(cnt, -residW[cnt]);
  cg(w, rho, rhs, prime);
...
VALUE SmallGoldenSectionLag(const VALUE left, const VALUE middle, const VALUE right, WVector &resid, WVector &residW,
                            const double *d, double *LogLik) {
  const VALUE GoldenRatio = (sqrt((double)5) - 1) / 2, GoldenToo = 1 - GoldenRatio;
  VALUE x0, x1, x2, x3, f0, f1, f2, f3;
  x0 = left;
  x3 = right;
...
VALUE SmallGoldenSectionLag(const VALUE left, const VALUE middle, const VALUE right, WVector &resid, WVector &residW,
                            const double *wr, const double *wi, double *LogLik) {
  const VALUE GoldenRatio = (sqrt((double)5) - 1) / 2, GoldenToo = 1 - GoldenRatio;
  VALUE x0, x1, x2, x3, f0, f1, f2, f3;
  x0 = left;
  x3 = right;
...
double SmallSimulationLag(Weights &W, int num_obs, const double rho, double *my_Y, double **my_X, const int deps,
  ...
  for (cnt = 0; cnt < dim; cnt++) lag.setAt(cnt, p_lag[cnt]);
...
// use __WXMAC__ to call vecLib
  ...
  if (!ordinaryLS(y, x, cov, resid, ols)) std::cerr << "svd error\n";      // ols = cov X' y
  if (!ordinaryLS(lag, x, cov, residW, olsW)) std::cerr << "svd error\n";  // olsW = cov X' lag
...
double SimulationLag(const geoda::GalElement *weight, int num_obs, int Precision, const double rho, double *my_Y,
  ...
  if (W.dim() < SMALL_DIM)
    return SmallSimulationLag(W, num_obs, rho, my_Y, my_X, deps, InclConstant, LogLik, false, p_bar_max_fraction,
                              p_bar_max_fraction);
...
VALUE SmallErrorLogLikelihood(Iterator<WVector> X, Iterator<WVector> lagX, WIterator y, WIterator lagY,
                              Iterator<WVector> W, const VALUE lambda, WVector &egls, double *d, bool InclConstant) {
  int cnt = 0;
  double lj = 0;
...
VALUE SmallErrorLogLikelihood(Iterator<WVector> X, Iterator<WVector> lagX, WIterator y, WIterator lagY,
                              Iterator<WVector> W, const VALUE lambda, WVector &egls, double *wr, double *wi,
                              bool InclConstant) {
  int cnt = 0;
  double jr = 1, ji = 0;
  for (cnt = 0; cnt < (int)y.count(); ++cnt)  // compute log-Jacobian
...
VALUE SmallGoldenSectionError(const VALUE left, const VALUE middle, const VALUE right, const WMatrix &X,
                              const WVector &y, Iterator<WVector> W, double *&beta, double *d, bool InclConstant,
                              double *LogLik) {
  const VALUE GoldenRatio = (sqrt((double)5) - 1) / 2, GoldenToo = 1 - GoldenRatio;
  VALUE x0, x1, x2, x3, f0, f1, f2, f3;
  x0 = left;
...
VALUE SmallGoldenSectionError(const VALUE left, const VALUE middle, const VALUE right, const WMatrix &X,
                              const WVector &y, Iterator<WVector> W, double *&beta, double *wr, double *wi,
                              bool InclConstant, double *LogLik) {
  const VALUE GoldenRatio = (sqrt((double)5) - 1) / 2, GoldenToo = 1 - GoldenRatio;
  VALUE x0, x1, x2, x3, f0, f1, f2, f3;
  x0 = left;
  x3 = right;
...
double SmallSimulationError(Weights &W, const double rho, const double *my_Y, double **my_X, const int deps,
  ...
  for (expl = 0; expl < deps; expl++) {
    for (cnt = 0; cnt < dim; ++cnt) X[expl] << my_X[expl][cnt];
  };
  X.reset(deps);
...
double SimulationError(const geoda::GalElement *my_gal, int num_obs, int Precision, const double rho,
  ...
  const int dim = W.dim();
  if (dim < SMALL_DIM)
    return SmallSimulationError(W, rho, my_Y, my_X, deps, beta, InclConstant, LogLik, false, p_bar_min_fraction,
                                p_bar_max_fraction);
  W.Transform(W_GWT);  // makes sure it is formated
  int cnt;
  WVector y(dim);
...
void sdiff(const SparseMatrix &w, const double rho, const DenseVector &v, DenseVector &d) {
  ...
  int cnt = 0;
  DenseVector vt(dim);
  for (cnt = 0; cnt < dim; ++cnt) vt.setAt(cnt, v.getValue(cnt) * w.getScale()[cnt]);
  DenseVector lag(dim);
  w.matrixColumn(lag, vt);

  for (cnt = 0; cnt < dim; ++cnt) d.setAt(cnt, (vt.getValue(cnt) - rho * lag.getValue(cnt)) / w.getScale()[cnt]);
}

double Lco(const DenseVector &y, const DenseVector &lag, const DenseVector *X, const SparseMatrix &w, const double rho,
           DenseVector &sol) {
  const int dim = w.dim();
...
double goldeno(const double left, const double right, const SparseMatrix &w, const DenseVector &y,
               const DenseVector &lag, const DenseVector *X) {
  const int dim = w.dim();
  ...
  if (f2 > f1) {  // x2 has value larger than x1 has
    f1 = f2;
    x1 = x2;
  };
  return x1;
...
double lcdfPlus(const DenseVector &x, const DenseVector &df) {
  ...
  for (int cnt = 0; cnt < x.getSize(); ++cnt) ss2 += geoda_sqr(x.getValue(cnt) + df.getValue(cnt));
  return (ss2 - ss) / ss;
}
...
double estLj(DenseVector &y, DenseVector &lag, const double rho, const SparseMatrix &w) {
  ...
  int cnt = 0;
  for (cnt = 0; cnt < dim; ++cnt) e.setAt(cnt, y.getValue(cnt) - rho * lag.getValue(cnt));
  double sss = 0;
  for (cnt = 0; cnt < dim; ++cnt) {
    works.reset();
    works.setAt(cnt, eps);
...
double getLj(const SparseMatrix &w, const double rho, const DenseVector &y, const DenseVector &lag) {
  ...
  int cnt = 0;
  for (cnt = 0; cnt < dim; ++cnt) e.setAt(cnt, (y.getValue(cnt) - rho * lag.getValue(cnt)) * w.getScale()[cnt]);
...
void EGLS(const double lambda, const DenseVector &y, const DenseVector *X, const SparseMatrix &w, DenseVector &egls) {
...
double mic(const DenseVector &resid, const DenseVector &residW, const double rho, const double trace,
           const double trace2) {
...
double mie(const DenseVector &rsd, const DenseVector &lag_resid, const double trace, const double trace2,
  ...
  for (cnt = 0; cnt < vars; ++cnt) {
    ...
    for (int cp = 0; cp < cnt; ++cp) cov[cnt][cp] = cov[cp][cnt] = dX[cnt].product(dX[cp]);
  ...
  for (cnt = 0; cnt < vars; ++cnt) s1.setAt(cnt, rsd.product(dX[cnt]) * dim / sse);  // s1 = partial dL/dBeta

  for (cnt = 0; cnt < vars; ++cnt) wk.setAt(cnt, -lag_resid.product(dX[cnt]) - rsd.product(lagX[cnt]));
  ...
  for (cnt = 0; cnt < vars; ++cnt) wk.setAt(cnt, rsd.product(dX[cnt]));
  ...
  for (cnt = 0; cnt < vars; ++cnt) wk.setAt(cnt, wk.getValue(cnt) + lagX[cnt].product(lag_resid));
