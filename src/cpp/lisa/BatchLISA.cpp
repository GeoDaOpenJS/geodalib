...
BatchLISA::BatchLISA(int num_obs, GeoDaWeight* w, const std::vector<std::vector<bool> >& _undefs,
...
int BatchLISA::GetSignificanceFilter() { return significance_filter; }

double BatchLISA::GetSignificanceCutoff() { return significance_cutoff; }

void BatchLISA::SetSignificanceCutoff(double val) { significance_cutoff = val; }

double BatchLISA::GetUserCutoff() { return user_sig_cutoff; }

void BatchLISA::SetUserCutoff(double val) { user_sig_cutoff = val; }

double BatchLISA::GetFDR(double current_p, int idx) {
  // bound check
  if (idx < 0 || idx >= (int)sig_local_vec.size() - 1) return 0;
...
int BatchLISA::GetNumPermutations() { return permutations; }

void BatchLISA::SetNumPermutations(int val) { permutations = val; }

uint64_t BatchLISA::GetLastUsedSeed() { return last_seed_used; }

bool BatchLISA::IsReuseLastSeed() { return reuse_last_seed; }

void BatchLISA::SetReuseLastSeed(bool reuse) { reuse_last_seed = reuse; }
...
bool BatchLISA::GetHasIsolates() { return has_isolates; }

bool BatchLISA::GetHasUndefined() { return has_undefined; }
...
std::vector<double> BatchLISA::GetLocalSignificanceValues(int idx) { return sig_local_vec[idx]; }

std::vector<int> BatchLISA::GetClusterIndicators(int idx) { return cluster_vec[idx]; }

std::vector<int> BatchLISA::GetSigCatIndicators(int idx) { return sig_cat_vec[idx]; }

std::vector<int> BatchLISA::GetNumNeighbors() { return nn_vec; }

std::vector<double> BatchLISA::GetSpatialLagValues(int idx) { return lag_vec[idx]; }

std::vector<double> BatchLISA::GetLISAValues(int idx) { return lisa_vec[idx]; }

bool BatchLISA::IsRowStandardize() const { return row_standardize; }

void BatchLISA::SetRowStandardize(bool rowStandardize) { row_standardize = rowStandardize; }

int BatchLISA::GetNumThreads() const { return nCPUs; }

void BatchLISA::SetNumThreads(int n_threads) { nCPUs = n_threads; }

std::vector<std::string> BatchLISA::GetLabels() { return labels; }

std::vector<std::string> BatchLISA::GetColors() { return colors; }
