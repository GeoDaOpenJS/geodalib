    ...
    if (undefs[i] || weights->IsMasked(i) == false) {
      ...
      if (weights->GetNbrSize(i) == 0) {
        ...
        if (data[i] > 0 && sp_lag > 0)
          ...
          cluster_vec[i] = CLUSTER_OTHERPOS;
        else if (data[i] < 0 && sp_lag < 0)
          cluster_vec[i] = CLUSTER_LOWLOW;
        else
          cluster_vec[i] = CLUSTER_NEGATIVE;  // data1[i] > 0 && sp_lag < 0
      }
    }
  }
}
...
uint64_t UniGeary::CountLargerSA(int cnt, const std::vector<double>& permutedSA) {
  ...
  if (lisa_vec[cnt] <= permGearyMean) {
    ...
    for (int i = 0; i < permutations; ++i) {
      ...
      if ((const unsigned int)cluster_vec[cnt] > CLUSTER_LOWLOW &&
          (const unsigned int)cluster_vec[cnt] < CLUSTER_UNDEFINED) {
    ...
    if ((const unsigned int)cluster_vec[cnt] < CLUSTER_UNDEFINED) {
