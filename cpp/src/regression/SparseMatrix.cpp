...
SparseMatrix::SparseMatrix(const geoda::GalElement *my_gal, int obs) { createGAL(my_gal, obs); }

void SparseMatrix::createGAL(const geoda::GalElement *my_gal, int obs) {  // get the weights from GAL file
  int dim = obs;
  ...
  for (cnt = 0; cnt < szi; ++cnt) ik[cnt] = -1;
  for (cnt = 0; cnt < dim; ++cnt) ik[key[cnt].first - key[0].first] = key[cnt].second;

  for (int r = 0; r < dim; ++r) {
    for (cnt = 0; cnt < row[r].getSize(); ++cnt) {
      int oix = row[r].getIx(cnt);
      if (oix < key[0].first || oix > key[dim - 1].first) {
        error("Error: value does not exist in the weights file");
        return;
      }
...
void SparseMatrix::WtTimesColumn(DenseVector &wtx, const DenseVector &x) {
  MakeTranspose();
  typedef std::list<std::pair<int, double> >::iterator l_itr_t;
