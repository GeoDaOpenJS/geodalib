...
Weights::Weights(const geoda::GalElement *my_gal, int num_obs) {
...
void SL_Transform(WVector &to, WMap::input_iterator from, const size_t dim) {
  ...
  for (; from; ++from) to[(*from).first] = (*from).second;
  return;
}
...
void SL_Transform(WVector &to, Iterator<INDEX> from, const size_t dim) {
  to.alloc(dim, 0);  // create zero array of size dim
  VALUE setvalue(1);
  for (; from; ++from) to[*from] = setvalue;
  return;
}
...
void SL_Transform(WMap &to, Iterator<INDEX> from, size_t cnt) {
  ...
  for (; from; ++from) to << WPair(*from, setvalue);
  return;
}
...
void Weights::Transform(const WeightsType otype) {
  ...
  if (format != otype) {
    switch (format) {
      case W_MAT:
        if (otype == W_GWT)
          SL_WeightsTransform(gt, mt());
        else
          SL_WeightsTransform(gl, mt());
        Destroy(mt());
        break;

      case W_GWT:
        if (otype == W_MAT)
          SL_WeightsTransform(mt, gt());
        else
          SL_WeightsTransform(gl, gt());
        Destroy(gt());
        break;

      case W_GAL:
        if (otype == W_MAT)
          SL_WeightsTransform(mt, gl());
        else
          SL_WeightsTransform(gt, gl());
        Destroy(gl());
        break;
...
void Weights::WeightsGal(const geoda::GalElement *my_gal, int num_obs) {
...
void Weights::MakeSymmetricStructure() {
  ...
  for (cnt = 0; cnt < dim; ++cnt) state << gt[cnt]();  // initialize state
...
void Weights::ComputeTranspose(GWT &transpose) {
  ...
  for (it = gt(); it; ++it)  // initialize counts
    for (row = (*it)(); row; ++row) ++count[(*row).first];
  ...
  for (cnt = 0; cnt < dim; ++cnt)
    for (row = gt[cnt](); row; ++row) transpose[(*row).first] << WPair(cnt, (*row).second);
  //    PrintM(transpose());
  return;
}
