...
void InitPoly(int precis, const int dim) {
  if (precis <= SL_SMALL_MATRIX) precis = SL_SMALL_MATRIX + 1;  // smaller precision is possible but who needs it?
...
inline void PermuteMatrix(Iterator<WVector> mt, const INDEX i, const INDEX j) {
  ...
  for (; mt; ++mt) {
    std::swap((*mt)[i], (*mt)[j]);  // exchange i and j columns
  }
  return;
}
...
void DanilevskyPreMultiply(Iterator<WVector> mt, const INDEX r1, WIterator its) {
  ...
  for (; mt; ++mt) {
    Row = *its++;
    if (Row != 0) {
      for (it = (*mt)(), itr = row1(); it; ++it) (*itr++) += *it * Row;
    }
  }
  for (itr = WIterator(row1.first() + r1); its; ++its) {
    (*itr++) += *its;
  }
  --mt;
  row1.Swap(*mt);
...
void PolyMultiply(WVector &a, WIterator b, WVector &c) {
  ...
  for (; ita; ++ita, ++itr) {
    VALUE vita = *ita;
    if (vita != 0)
      for (itb = b, itr2 = itr; itb; ++itb) *itr2++ -= vita * (*itb);
  };
  return;
}
 ...
 */
// template <class M>
INDEX LevelStructure(Iterator<WMap> mt, INDEX &start, INDEX &Contig, const Iterator<INDEX> perm, INDEX *order) {
    ...
    do {
      ...
      for (current = first; current < last; ++current)
        for (iROW r = mt[*current](); r; ++r) {
          INDEX tid = (*r).first;
          if (order[tid] == Dim1) order[(*next++) = tid] = conty;
        };
  ...
  if (pieces == 1) {  // find element with min degree
    init = Dim1;
    //    cout << "ordering " << std::setw(4) << mt.count() << "  elts,  conty: " << std::setw(4) << conty << "  has "
    //    << last-first << std::endl;
    for (current = first; current < last; ++current) {
...
void PolyProduct(Vector<P> &v1, Iterator<P> v2) {
  ...
  if (Restricted) {
    ...
    for (; First < it1; --it1) {
      if ((Current = *it1) != 0)
        for (*it1 = 0, it2 = v2, work = it1; it2; ++it2) (*work++) += *it2 * Current;
      v2.inflate();
    };
  };
  ...
  for (; !(it1 < First); --it1)
    for (Current = *it1, *it1 = 0, it2 = v2, work = it1; it2; ++it2) (*work++) += *it2 * Current;
...
void SparseSubMatrix(Iterator<WMap> mt, Iterator<INDEX> perm, INDEX *flag, Vector<WMap> &v) {
  ...
  for (; v; ++v) {
    ...
    for (; OneRow; ++OneRow)
      if (flag[(*OneRow).first] == FlagId) *v << WMap::data(InversePermute[(*OneRow).first], (*OneRow).second);
  };
  return;
}
...
void SavePerm(Iterator<IR> it, INDEX *perm) {
  for (; it; ++it) (*it).first = perm[(*it).first];
}
...
/*
        SelectMatrix
 */
inline void SelectMatrix(Iterator<WMap> mt, const INDEX start, const INDEX stop, Vector<WMap> &v) {
...
void SparsePolyBordering(Iterator<WMap> mt, const INDEX BeltSize, WMatrix &Poly) {
  ...
  for (; InTheBelt < Dim; ++InTheBelt) {
    ...
    if (symmetric) {
      ...
      *Poly << 1 << -Diag << -Lag;
      while (*Poly) *Poly << -SparseLag.ComputeLag();
  ...
  if (dim <= 1) {
    ...
    *Poly << 1 << -val;
    ...
    for (; mt; ++mt, ++BufferDense) {
      ...
      for (itr = (*mt)(); itr; ++itr) (*BufferDense)[(*itr).first] = (*itr).second;
    };
    tmp = BufferDense();
    ...
    for (; mt; ++mt, ++Dense) {
      (*Dense).alloc(dim, 0);
      for (itr = (*mt)(); itr; ++itr) (*Dense)[(*itr).first] = (*itr).second;
    };
...
INDEX SplitSparse(Iterator<WMap> mt, INDEX Pseudo, INDEX &Second, INDEX &PseudoCount) {
  ...
  do {
    for (n1 = 0; n1 < dim; ++n1) order[n1] = dim + 1;
    oldPseudo = Pseudo;
  ...
  if (localLevel == 1)
    ...
    while (order[perm[--n2]] == localLevel - 1) ++count;
  ...
  for (n1 = 0; n1 < dim; ++n1) InvPerm[perm[n1]] = n1;
...
void PartialRename(Iterator<WMap> it, const INDEX Deduct) {
  for (; it; ++it)
    for (Iterator<WPair> itr = (*it)(); itr; ++itr) (*itr).first -= Deduct;
  return;
}
