...
double product(const double *v1, const double *v2, const int &sz) {
  double s = 0;
  for (int cnt = 0; cnt < sz; ++cnt) s += v1[cnt] * v2[cnt];
  return s;
}
...
double *vproduct(double *v1, double *v2, int dim) {
  ...
  for (int i = 0; i < dim; i++) rslt[i] = v1[i] * v2[i];
...
double gcf(const double a, const double x) {
  ...
  for (int n = 1; n <= ITMAX; ++n) {
    ...
    if (fabs(a1) > 0.0) {
      fac = 1.0 / a1;
      g = b1 * fac;
      if (fabs((g - gold) / g) < EPS) return exp(-x + a * log(x) - gln) * g;
      gold = g;
    };
  };
  return 0;
...
double *BP_Test(double *resid, int obs, double **X, int nvar, bool InclConst) {
  ...
  for (i = 0; i < nvar; i++) alloc(cov[i], nvar);
...
// #ifdef WORDS_BIGENDIAN
  ...
  for (i = 0; i < nvar; i++) gz.setAt(i, g.product(x[i]));  // gz = g'z (1 x expl)
...
double MC_Condition_Number(double **X, int dim, int expl) {
  ...
  for (i = 0; i < expl; i++) x[i].absorb(X[i], dim, false);
  ...
  for (row = 0; row < expl; row++) {
    for (column = row + 1; column < expl; column++) a[row + column * (column + 1) / 2] = x[row].product(x[column]);
...
double *WhiteTest(int obs, int nvar, double *resid, double **X, bool InclConstant) {
  ...
  for (jj = 0; jj < obs; jj++) w[0][jj] = 1.0;

  if (InclConstant) {
    for (i = 1; i < nvar; i++) {
      for (jj = 0; jj < obs; jj++) w[i][jj] = X[i][jj];
    }
  } else {
    for (i = 0; i < nvar; i++)
      for (jj = 0; jj < obs; jj++) w[i + 1][jj] = X[i][jj];
  }

  // (4) Create cross product of Xs and store them into w[nvar ... df][]
  for (i = 1 - ix; i < nvar; i++) {
    for (int j = i; j < nvar; j++) {
      for (jj = 0; jj < obs; jj++) w[k][jj] = X[i][jj] * X[j][jj];
      k++;
    }
  }
  ...
  double s_u = 0.0;
  for (i = 0; i < obs; i++) s_u += geoda_sqr(r2[i] - r2_bar);
