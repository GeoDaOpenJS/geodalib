...
void MakeFastLookupMat(geoda::GalElement *g, int dim, std::vector<std::set<int> > &g_lookup) {
  g_lookup.resize(dim);
...
double T(geoda::GalElement *g, int dim) {
  // tr(W'W+WW)
  // = tr(W'W) + tr(WW)
...
// double T(const geoda::GalElement *g, int dim)
//{
//   double	sum = 0;
//    int cnt = 0, cp = 0;
//    for (cnt = 0; cnt < dim; ++cnt)
                     ...
                     double t)  // t = T(g, dim, g_lookup)
{
  ...
  for (cnt = 0; cnt < expl; ++cnt) z.setAt(cnt, x[cnt].product(lag));  // z = X'WXb
...
//
void Compute_RSLmLagRobust(geoda::GalElement *g, double **cov, DenseVector y, DenseVector *x, DenseVector ols, double *resid,
                           int dim, int expl, double *rst,
                           double T21)  // T21 = T(g, dim, g_lookup)
{
  ...
  for (cnt = 0; cnt < expl; ++cnt) z.setAt(cnt, x[cnt].product(Wy));  // z = X'WXb
...
void ReportDenseVector(const std::string ttl, DenseVector *X, int n, int k) {
  char buf[222];
  std::string msg = ttl + ":\n";
  for (int i = 0; i < n; i++) {
    for (int j = 0; j < k; j++) {
...
double Compute_MoranZ(geoda::GalElement *g,
  ...
  for (int i = 0; i < n; i++) {
    ...
    for (std::map<int, bool>::iterator B_it = B[i].begin(); B_it != B[i].end(); ++B_it) {
      int j = B_it->first;
      it = W_map[i].find(j);
...
void Compute_RSLmErrorRobust(geoda::GalElement *g, double **cov, DenseVector y, DenseVector *x, DenseVector ols, double *resid,
                             int dim, int expl, double *rst,
...
void Compute_RSLmSarma(geoda::GalElement *g, double **cov, DenseVector y, DenseVector *x, DenseVector ols, double *resid,
                       int dim, int expl, double *rst, double T21) {
  double *Y = y.getThis();
  double const ee = norm(resid, dim);
  double const sigma2 = ee / (dim);
...
extern double mic(const DenseVector &resid, const DenseVector &residW, const double rho, const double trace,
                  const double trace2);
...
bool geoda::classicalRegression(geoda::GalElement *g, int num_obs, double *Y, int dim, double **X, int expl, DiagnosticReport *dr,
  ...
  if (!InclConstant) {
    double e_bar = 0;
    for (cnt = 0; cnt < dim; cnt++) e_bar += resid[cnt];
    e_bar /= dim;
    ...
    for (cnt = 0; cnt < n; cnt++) sum_y += geoda_sqr(y.getValue(cnt) - e_bar);

    R2 = 1.0 - (e2 / (sum_y));

  } else {
    for (cnt = 0; cnt < n; cnt++) sum_y += geoda_sqr(y.getValue(cnt) - ybar);
...
bool geoda::spatialLagRegression(geoda::GalElement *g, int num_obs, double *Y, int dim, double **X, int deps, DiagnosticReport *dr,
  ...
  for (cnt = 0; cnt < deps; ++cnt) x[cnt].absorb(X[cnt], dim, false);
  ...
  for (int vr = 0; vr < deps; ++vr) xbeta.addTimes(x[vr], beta.getValue(vr));
  ...
  for (cnt = 0; cnt < deps + 2; ++cnt) info_matrix[cnt] = new double[deps + 2];
  ...
  for (cnt = 0; cnt < dim; ++cnt) sol.setAt(cnt, sol.getValue(cnt) / orig.getScale()[cnt]);
  ...
  for (cnt = 0; cnt < n; ++cnt) works.setAt(cnt, works.getValue(cnt) / orig.getScale()[cnt]);

  for (cnt = 0; cnt < deps; ++cnt) info_matrix[cnt][deps] = info_matrix[deps][cnt] = x[cnt].product(works) / sigma2;
  ...
  for (cnt = 0; cnt < deps; ++cnt) info_matrix[deps + 1][cnt] = info_matrix[cnt][deps + 1] = 0.0;
  ...
  if (!InclConstant) {
    double e_bar = 0;
    for (cnt = 0; cnt < n; cnt++) e_bar += rfin.getValue(cnt);
    e_bar /= n;
...
bool geoda::spatialErrorRegression(geoda::GalElement *g, int num_obs, double *Y, int dim, double **XX, int deps, DiagnosticReport *rr,
  ...
  DenseVector y(Y, dim, false), *X = new DenseVector[deps];
  int cnt = 0, row = 0, column = 0;
  for (cnt = 0; cnt < deps; ++cnt) X[cnt].absorb(XX[cnt], dim, false);

  double *beta;
  const int n = dim;

  double LogLike = 0, initLambda = 0;
  initLambda = SimulationError(g, num_obs, 100, 0.31, Y, XX, deps, beta, !InclConstant, &LogLike, 0.0, 0.1);
  release(&beta);
  ...
  orig.makeStdSymmetric();
  run1(orig, initLambda, trace, trace2, fr, 0.1, 0.55);
  orig.makeRowStd();
  ...
  run1(orig, lambda, trace, trace2, fr, 0.55, 1);
  orig.makeRowStd();
  ...
  for (int vr = 0; vr < vars; ++vr) xbeta.addTimes(X[vr], egls.getValue(vr));

  for (cnt = 0; cnt < dim; ++cnt) {
    //		rr->SetResidual(cnt,rsd.getValue(cnt));
    rr->SetPredErr(cnt, Y[cnt] - xbeta.getValue(cnt));
    rr->SetResidual(cnt, Y[cnt] - xbeta.getValue(cnt) - (lambda * lag_resid.getValue(cnt)));
  ...
  for (cnt = 0; cnt < vars; ++cnt) {
    ...
    for (int rc = cnt + 1; rc < vars; ++rc) {
      info_matrix[cnt][rc] = info_matrix[rc][cnt] = lag_X[cnt].product(lag_X[rc]) / sigma2;
    }
  }

  // d2L/ds2ds2
  info_matrix[vars + 1][vars + 1] = 0.5 * dim / sigma2 / sigma2;
  for (cnt = 0; cnt < vars; ++cnt) info_matrix[vars + 1][cnt] = info_matrix[cnt][vars + 1] = 0.0;
  // d2L/ds2dl
  ...
  if (!InclConstant) {
    double e_bar = 0;
    for (cnt = 0; cnt < n; cnt++) e_bar += rsd.getValue(cnt);
    e_bar /= n;
