#ifndef RANGE_ADJUST_H
#define RANGE_ADJUST_H
 ...
 */
inline void range_adjust(std::vector<double>& data, std::vector<bool>& undef) {
  // Validate input
  ...
  if (!has_valid_data) {
    throw std::runtime_error("All values are undefined - cannot perform range adjustment");
  }
...
inline void range_adjust_wasm(std::vector<double>& data, const std::vector<unsigned int>& undef) {
  std::vector<bool> undef_bool(undef.size());
  for (size_t i = 0; i < undef.size(); ++i) {
    undef_bool[i] = undef[i] != 0;
  }
  range_adjust(data, undef_bool);
}

}  // namespace geoda
#endif  // RANGE_ADJUST_H
