...
UniLocalMoran::UniLocalMoran(int num_obs, GeoDaWeight* w, const std::vector<double>& _data,
                             const std::vector<bool>& _undefs, double significance_cutoff, int _nCPUs, int _perm,
  ...
  data = geoda::standardize_data(data, undefs);
  Run();
}
  ...
  for (int i = 0; i < num_obs; i++) {
    if (undefs[i] || weights->IsMasked(i) == false) {
      ...
      if (weights->GetNbrSize(i) == 0) {
        ...
        // assign the cluster
        if (data[i] > 0 && sp_lag < 0)
          cluster_vec[i] = CLUSTER_HIGHLOW;
        else if (data[i] < 0 && sp_lag > 0)
