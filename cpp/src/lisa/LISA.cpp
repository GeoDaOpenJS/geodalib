...
LISA::LISA(int num_obs, GeoDaWeight* w, const std::vector<bool>& _undefs, double _significance_cutoff, int _nCPUs,
           int _perm, const std::string& _permutation_method, uint64_t _last_seed)
    : nCPUs(_nCPUs),
      num_obs(num_obs),
...
LISA::LISA(int num_obs, GeoDaWeight* w, const std::vector<std::vector<bool> >& _undefs, double _significance_cutoff,
...
int LISA::GetSignificanceFilter() { return significance_filter; }

double LISA::GetSignificanceCutoff() { return significance_cutoff; }

void LISA::SetSignificanceCutoff(double val) { significance_cutoff = val; }

double LISA::GetUserCutoff() { return user_sig_cutoff; }

void LISA::SetUserCutoff(double val) { user_sig_cutoff = val; }
...
double LISA::GetFDR(double current_p) {
  ...
  for (int i = 0; i < num_obs; i++) {
    double val = (i + 1) * current_p / num_obs;
    if (i == 0) fdr = val;
    if (pvals[i] >= val) {
      break;
    }
...
double LISA::GetBO(double current_p) {
  ...
  double bonferroni_bound = current_p / num_obs;
  return bonferroni_bound;
}

int LISA::GetNumPermutations() { return permutations; }
void LISA::SetNumPermutations(int val) { permutations = val; }

uint64_t LISA::GetLastUsedSeed() { return last_seed_used; }

bool LISA::IsReuseLastSeed() { return reuse_last_seed; }

void LISA::SetReuseLastSeed(bool reuse) { reuse_last_seed = reuse; }
...
bool LISA::GetHasIsolates() { return has_isolates; }

bool LISA::GetHasUndefined() { return has_undefined; }
...
void LISA::PermCreateTable_threaded() {
  ...
  for (int i = 0; i < tot_threads; i++) {
    ...
    uint64_t seed_start = last_seed_used + a * (permutations /*larger enougth to avoid reoccurence*/ * max_neighbors);
#ifndef __USE_PTHREAD__
    boost::thread* worker =
        new boost::thread(boost::bind(&LISA::PermCreateRange, this, a, b, max_neighbors, seed_start));
...
void LISA::PermCreateRange(int perm_start, int perm_end, int max_neighbors, uint64_t seed_start) {
  ...
  for (int cnt = perm_start; cnt <= perm_end; cnt++) {
    ...
    while (rand < max_neighbors) {
      ...
      rng_val = geoda::ThomasWangHashDouble(seed_start++) * max_rand;
      // round is needed to fix issue
      // https://github.com/GeoDaCenter/geoda/issues/488
      newRandom = static_cast<int>(rng_val < 0.0 ? ceil(rng_val - 0.5) : floor(rng_val + 0.5));
...
void LISA::PermCalcPseudoP_range(int obs_start, int obs_end, uint64_t seed_start) {
  for (int cnt = obs_start; cnt <= obs_end; cnt++) {
    ...
    if (_sigLocal <= 0.0001)
      sig_cat_vec[cnt] = 4;
    else if (_sigLocal <= 0.001)
      sig_cat_vec[cnt] = 3;
    else if (_sigLocal <= 0.01)
      sig_cat_vec[cnt] = 2;
    else if (_sigLocal <= 0.05)
      sig_cat_vec[cnt] = 1;
    else
      sig_cat_vec[cnt] = 0;
...
void LISA::CalcPseudoP_range(int obs_start, int obs_end, uint64_t seed_start) {
  ...
  for (int cnt = obs_start; cnt <= obs_end; cnt++) {
    ...
    // NOTE: undefined should be marked as well, however, since undefined_cat has covered undefined category, we don't
    // need to handle here
  }
}
...
std::vector<double> LISA::GetLocalSignificanceValues() { return sig_local_vec; }

std::vector<int> LISA::GetClusterIndicators() { return cluster_vec; }

std::vector<int> LISA::GetSigCatIndicators() { return sig_cat_vec; }

std::vector<int> LISA::GetNumNeighbors() { return nn_vec; }

std::vector<double> LISA::GetSpatialLagValues() { return lag_vec; }

std::vector<double> LISA::GetLISAValues() { return lisa_vec; }

bool LISA::IsRowStandardize() const { return row_standardize; }

void LISA::SetRowStandardize(bool rowStandardize) { row_standardize = rowStandardize; }

int LISA::GetNumThreads() const { return nCPUs; }

void LISA::SetNumThreads(int n_threads) { nCPUs = n_threads; }

std::vector<std::string> LISA::GetLabels() { return labels; }

std::vector<std::string> LISA::GetColors() { return colors; }
