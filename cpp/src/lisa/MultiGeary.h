...
class MultiGeary : public LISA {
  ...
  const unsigned int CLUSTER_POSITIVE;
  const unsigned int CLUSTER_NEGATIVE;
  const unsigned int CLUSTER_UNDEFINED;
  const unsigned int CLUSTER_NEIGHBORLESS;

 public:
  MultiGeary(int num_obs, GeoDaWeight* w, const std::vector<std::vector<double> >& data,
             const std::vector<std::vector<bool> >& undefs, double significance_cutoff, int nCPUs, int permutations,
