...
BiLocalMoran::BiLocalMoran(int num_obs, GeoDaWeight* w, const std::vector<double>& _data1,
                           const std::vector<double>& _data2, const std::vector<bool>& _undefs,
  ...
  data1 = geoda::standardize_data(data1, undefs);
  data2 = geoda::standardize_data(data2, undefs);
  Run();
}
  ...
  for (int i = 0; i < num_obs; i++) {
    if (undefs[i] || weights->IsMasked(i) == false) {
      ...
      if (weights->GetNbrSize(i) == 0) {
        ...
        // assign the cluster
        if (data1[i] > 0 && sp_lag < 0)
          cluster_vec[i] = CLUSTER_HIGHLOW;
        else if (data1[i] < 0 && sp_lag > 0)
          cluster_vec[i] = CLUSTER_LOWHIGH;
        else if (data1[i] < 0 && sp_lag < 0)
          cluster_vec[i] = CLUSTER_LOWLOW;
        else
          cluster_vec[i] = CLUSTER_HIGHHIGH;  // data1[i] > 0 && Wdata > 0
      }
    }
  }
}
...
std::vector<int> BiLocalMoran::GetClusterIndicators() {
  ...
  for (int i = 0; i < num_obs; i++) {
    if (sig_local_vec[i] > cuttoff && (const unsigned int)cluster_vec[i] != CLUSTER_UNDEFINED &&
        (const unsigned int)cluster_vec[i] != CLUSTER_NEIGHBORLESS) {
