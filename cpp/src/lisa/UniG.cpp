...
UniG::UniG(int num_obs, GeoDaWeight* w, const std::vector<double>& _data, const std::vector<bool>& _undefs,
...
void UniG::PermLocalSA(int cnt, int perm, int numNeighbors, const int* permNeighbors, std::vector<double>& permutedSA) {
...
void UniG::PermLocalSA(int cnt, int perm, const std::vector<int>& permNeighbors, std::vector<double>& permutedSA) {
  int validNeighbors = 0;
  double permutedLag = 0;
...
std::vector<int> UniG::GetClusterIndicators() {
  ...
  for (int i = 0; i < num_obs; i++) {
    if ((const unsigned int)cluster_vec[i] == CLUSTER_UNDEFINED &&
