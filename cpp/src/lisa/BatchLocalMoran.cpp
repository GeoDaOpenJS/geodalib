...
BatchLocalMoran::BatchLocalMoran(int num_obs, GeoDaWeight *w, const std::vector<std::vector<double> > &_data,
                                 const std::vector<std::vector<bool> > &_undefs, double _significance_cutoff,
  ...
  for (int i = 0; i < num_batch; ++i) {
    data[i] = geoda::standardize_data(data[i], undefs[i]);
  }
    ...
    for (int i = 0; i < num_obs; i++) {
      if (undefs[v][i]) {
        ...
        if (weights->GetNbrSize(i) == 0) {
          ...
          double sp_lag = 0;
          const std::vector<unsigned int> &nbrs = weights->GetNeighbors(i);
          unsigned int nn = 0;
          ...
          // assign the cluster
          if (data[v][i] > 0 && sp_lag < 0)
            cluster_vec[v][i] = CLUSTER_HIGHLOW;
          else if (data[v][i] < 0 && sp_lag > 0)
            cluster_vec[v][i] = CLUSTER_LOWHIGH;
          else if (data[v][i] < 0 && sp_lag < 0)
            cluster_vec[v][i] = CLUSTER_LOWLOW;
          else
            cluster_vec[v][i] = CLUSTER_HIGHHIGH;  // data1[i] > 0 && Wdata > 0
        }
      }
    }
  }
...
std::vector<int> BatchLocalMoran::GetClusterIndicators(int idx) {
  ...
  for (int i = 0; i < num_obs; i++) {
    if (sig_local_vec[idx][i] > cuttoff && (const unsigned int)cluster_vec[idx][i] != CLUSTER_UNDEFINED &&
        (const unsigned int)cluster_vec[idx][i] != CLUSTER_NEIGHBORLESS) {
